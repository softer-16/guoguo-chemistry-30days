...
- 当前阶段采用用户自主注册：用户创建账号后，通过闲鱼订单消息发送注册邮箱；管理员再手动授予课程权限。不包含支付或管理员后台。
...
4. 已付款用户在网站创建账号后，通过闲鱼订单消息发送注册邮箱；管理员在 SQL Editor 中为该账号授予课程权限。
...
上线前必须将正式网站地址配置为 Supabase Auth 的 Site URL，并添加到 Redirect URLs。密码重置还需要将正式网站的 `https://你的域名/?reset-password=1` 加入 Redirect URLs。网页不写死个人联系方式；未开通用户统一通过闲鱼订单消息联系商家，并提供注册邮箱。

## 注册、登录与开通状态

- 注册成功后，网页提示“账号创建成功，请直接使用注册邮箱和密码登录。已付款用户请通过购买平台发送注册邮箱，等待管理员开通课程。”并提供去登录按钮；注册不会自动进入课程。
- 邮箱或密码不正确时只显示通用提示，不透露账号是否存在；网络或登录服务异常时提示稍后重试。
- 登录成功但没有 `active` 授权时，网页显示“已登录，课程正在等待管理员开通”；管理员授予 `active` 后，用户登录即可进入完整 Day01–Day30 课程。
